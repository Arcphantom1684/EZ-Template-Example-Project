...
bool override_intake = false;    // toggled with button A
bool pto_in_drive_mode = false;  // false = intake, true = drive
...
while (true) {
    ...
    if (master.get_digital_new_press(DIGITAL_A)) {
        override_intake = !override_intake;
    }

    // PTO target
    bool should_be_drive = !(master.get_digital(DIGITAL_R2) || master.get_digital(DIGITAL_R1) ||
                             master.get_digital(DIGITAL_L1) || master.get_digital(DIGITAL_L2));

    if (override_intake) {
        // Override: force PTO to intake position
        should_be_drive = false;
    }

    // Shift PTO if needed
    if (should_be_drive != pto_in_drive_mode && !pto_changing) {
        pto_in_drive_mode = should_be_drive;
        pto.set_value(pto_in_drive_mode);
        pto_timer = pros::millis();
        pto_changing = true;
    }

    if (pto_changing && pros::millis() - pto_timer >= PTO_DELAY_MS) {
        pto_changing = false;
    }
    ...
    if (!pto_changing) {
        if (!pto_in_drive_mode) { // intake mode or buttons control intake/flywheel
            ...
            else if (master.get_digital(DIGITAL_L2)) { intake.move(127); flywheel.move(0); }
            else { intake.move(0); flywheel.move(0); }
        } else { // drive mode → joystick control
            intake.move(-master.get_analog(ANALOG_LEFT_Y));
